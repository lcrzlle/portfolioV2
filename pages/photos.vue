<template>
	<main id="pageContent">
		<div id="pageTitle" ref="pageTitleRef">
			<LayoutPageTitle pageTitle="Photos" subtitleOne="Projets" subtitleTwo="Galerie"
				subtitleThree="Portfolio" />
		</div>
		<ul id="pageSlider">
			<li v-for="(item, index) in projectsSlider.slice().reverse()" :key="index" class="slider__item__wrapper">
				<div>
	...
	<Teleport to="body">
		<div class="slider__labels" :class="{ 'is-visible': showLabels }">
			<span v-for="(item, index) in projectsSlider.slice().reverse()" :key="index" class="slider__item__label">
	...
	<LayoutFooter>
		<ul class="footer__thumb__inner">
			<li v-for="item in projectsSlider.slice().reverse()">
...
function handleItem(event) {
	...
	itemLink.value = '/photos/' + slug;
	const project = projectsSlider.value.find(p => p.slug === slug);
